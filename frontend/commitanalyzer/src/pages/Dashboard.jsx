...
const Dashboard = () => {
    ...
    return (
        ...
        <div className='relative w-full min-h-screen'>
            <Tree owner={owner} repo={repo} url={url} updateUrl={setUrl} page={page} setPage={setPage} setLoading={setLoading} fromRefresh={fromRefresh}/>
            <div className='flex gap-15 absolute top-18 w-full justify-center lg:pl-10 max-sm:p-0 sm:p-0 '>
                <form className='' onSubmit={handleSubmit}>
                    <div className='h-22 py-5 pl-7 max-sm:w-80 sm:w-95 lg:w-160 bg-white/10 backdrop-blur-2xl border border-white/20 shadow-[0_8px_32px_0_rgba(38,166,65,0.37)] rounded-2xl '>
                        <input className='bg-neutral-800 max-sm:w-50 sm:w-60 lg:w-120 p-2 py-3 rounded focus: outline-green-600 mr-3' autoComplete="off"  type="text" id='url' placeholder='https://github.com/username/repository' onChange={(e) => setUrl(e.target.value)}  value={url ? url : 'https://github.com/'}/>
                        <button className={`bg-green-600 px-4 py-3 rounded hidden max-sm:inline ${loading ? 'cursor-not-allowed' : 'cursor-pointer'}`} type='submit' disabled={loading}>Go</button>
                        <button className={`bg-green-600 px-4 py-3 rounded max-sm:hidden ${loading ? 'cursor-not-allowed' : 'cursor-pointer'}`} type='submit' disabled={loading}>{loading ? 'Analyzing...' : 'Analyze'}</button>
                        
                    </div>
                </form>
                <div className=' h-22 w-50 p-2 pl-5 flex items-center bg-white/10 backdrop-blur-2xl border border-white/20 shadow-[0_8px_32px_0_rgba(38,166,65,0.37)] rounded-2xl max-sm:hidden'>
                    <RefreshCw className={`mr-5 ${loading ? 'cursor-not-allowed' : 'cursor-pointer'}`} onClick={(e) => {handleSubmit(e)
                    setFromRefresh(true)}} disabled={loading}/>
                    <button className={`bg-green-600 px-4 py-3 rounded ${loading ? 'cursor-not-allowed' : 'cursor-pointer'}`} onClick={() => {setPage(prev => prev+1)}}disabled={loading}>{loading ? 'Loading...' : 'Load More'}</button>
                </div>
